define(['rdfjson/Rdfs'], function (Rdfs) {
    ...
    var convert = function (graph, conf) {
        ...
        if (!conf.ignoreAllClasses) {
            ...
            for (var c = 0; c < clss.length; c++) {
                ...
                for (var p = 0; p < cats[0].length; p++) {
                    ...
                    //console.log("---> " + getAbbrevId(cats[0][p].getURI(), conf));
                    if (!propArr.includes(getAbbrevId(cats[0][p].getURI(), conf), propArr)) {
                        propArr.push(getAbbrevId(cats[0][p].getURI(), conf));
                    }
                }
                    ...
                    for (var q = 1; q < cats.length; q++) {
                        ...
                        if (cat.length > 0) {
                            ...
                            for (var s = 0; s < cat.length; s++) {
                                ...
                                //console.log("---> " + getAbbrevId(cat[s].getURI(), conf));
                                if (!group.items.includes(getAbbrevId(cat[s].getURI(), conf), group.items)) {
        ...
        if (conf.root) {
            return { templates: auxP.concat(auxC), scope: conf.abbrev, namespace: conf.ns, root: conf.root };
        } else {
