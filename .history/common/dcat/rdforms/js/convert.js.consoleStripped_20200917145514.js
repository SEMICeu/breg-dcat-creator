function includedcatnamespace (text) {
    setTimeout( function () {
        if (( text.split("rdf:about").length - 1 < 2 )) {
            if (( text.split('xmlns:dcat="http://purl.org/dc/terms/"').length - 1 < 1 )) {
                xmlns:dcat="http://purl.org/dc/terms/";
                var textXML = dijit.byId("dijit_form_SimpleTextarea_1").get("value");
                posStartTag = textXML.indexOf("<rdf:RDF ");
                textXML = textXML.substr(0, posStartTag + 9) + 'xmlns:dcat="http://purl.org/dc/terms/"' + "\n    " + textXML.substr(posStartTag + 9);
...
function convertToCPSV () { 
    setTimeout( function () {
        ...
        includedcatnamespace(textXML.get("value"));   
...
function transformRDFXMLtoCPSV(text) {
    ...
    var posStartDescription = text.indexOf('<rdf:Description rdf:about="http://www.w3.org/ns/dcat#">');
    var posIdentifier = text.indexOf("<dcat:identifier", posStartDescription);
    ...
    if (posStartDescription != -1 && posIdentifier != -1 && posEndDescription != -1 && posStartDescription < posIdentifier && posIdentifier < posEndDescription) {
        ...
        /*insert <rdf:type rdf:resource="http://www.w3.org/ns/dcat#Catalog"/>*/
        text = text.substr(0, posEnd + 21) + "\n" + '    <rdf:type rdf:resource="http://www.w3.org/ns/dcat#Catalog"/>' + text.substr(posEnd + 21);
        /*replace with identifier*/
        if (isURL(identifier)) { 
            text = text.replace("http://www.w3.org/ns/dcat#", identifier);
        } else {
            text = text.replace('rdf:about="http://www.w3.org/ns/dcat#', 'rdf:nodeID="' + identifier);
        }
    /* if no identifier is present for the PS */       
        ...
        text = text.replace('rdf:about="http://www.w3.org/ns/dcat#', 'rdf:nodeID="PS_0');
        /*insert <rdf:type rdf:resource="http://www.w3.org/ns/dcat#Catalog"/>*/
        index = text.indexOf('rdf:nodeID="PS_0">');
        text = text.substr(0, index + 18) + "\n" + '    <rdf:type rdf:resource="http://www.w3.org/ns/dcat#Catalog"/>' + "\n" + '    <dcat:identifier>PS_0</dcat:identifier>' + text.substr(index + 18);
    } else {
    ...
    while (text.indexOf("<dcat:identifier", posEndDescription) != -1 ) {
        /*find identifier value*/
        pos = text.indexOf("dcat:identifier", posEndDescription);
...
function transformRDFXMLtoEditor(text) {
    ...
    var posPS = text.indexOf('<rdf:type rdf:resource="http://www.w3.org/ns/dcat#Catalog"/>');
    ...
    if ( posPS != -1 && posStartDescription != -1 && posIdentifier != -1 && posStartDescription < posIdentifier && posIdentifier < posPS ) {
        ...
        text = text.replace(identifier, "http://www.w3.org/ns/dcat#");
        /*remove <rdf:type rdf:resource="http://www.w3.org/ns/dcat#Catalog"/>*/
        text = text.replace('<rdf:type rdf:resource="http://www.w3.org/ns/dcat#Catalog"/>', "");
        text = text.replace(/^\s*[\r\n]/gm, "");
    /* if blank node identifier for PS */
        ...
        var identifier = text.substring(posStart, posEndID);
        /*replace with example:about*/
        text = text.replace('rdf:nodeID="' + identifier, 'rdf:about="http://www.w3.org/ns/dcat#');
        /*remove <rdf:type rdf:resource="http://www.w3.org/ns/dcat#Catalog"/>*/
        text = text.replace('<rdf:type rdf:resource="http://www.w3.org/ns/dcat#Catalog"/>', "");
        text = text.replace(/^\s*[\r\n]/gm, "");
    } else {
